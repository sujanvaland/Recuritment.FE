...
export default function SettingsPage() {
  ...
  // Update your useEffect to populate account fields
  useEffect(() => {
    // Populate account fields from user context
    if (user) {
      setFirstName(user.firstName || "");
      setLastName(user.lastName || "");
      setEmail(user.email || "");
      setPhone(user.recuriterPhone || ""); // Use phone first, fallback to recuriterPhone
      setJobTitle(user.title || "");
      setProfilePicture( user.profileImageUrl || ""); // Use profilePicture first, fallback to ProfileImageUrl
    }
  }, [user]);

  // Add this useEffect to fetch notification settings
  useEffect(() => {
    const fetchNotificationSettings = async () => {
      ...
      try {
        ...
        if (response.status === 200 && response.data?.notifications) {
          ...
          // Bind notification preferences from API
          setEmailNotifications(notifications.emailNotifications || false);
          setApplicationUpdates(notifications.applicationUpdates || false);
          setJobAlerts(notifications.jobAlerts || false);
          setMessageNotifications(notifications.messageNotifications || false);
          setInterviewReminders(notifications.interviewReminders || false);
...
debugger;
  ...
  const handleProfilePictureUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fileService.uploadfile(file, "profilepicture");
     debugger;
      if (response && typeof response === 'string') {
        setProfilePicture(response); // Use response directly, not response.length
  ...
  const resetNotificationsToDefault = async () => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        alert("Notification preferences reset to default and saved!");
      }
    } catch (error) {
      alert("Preferences reset locally. Click 'Save Preferences' to save to server.");
    }
  };
